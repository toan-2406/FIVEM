$(document).ready(function () {
	...
	function setupImpoundForm(data) {
		...
		$('#weeks').attr("placeholder", `0 - ${rules.maxWeeks} tuần`);
		$('#days').attr("placeholder", `0 - ${rules.maxDays} ngày`);
		$('#hours').attr("placeholder", `0 - ${rules.maxHours} giờ`);
		$('#fee').attr("placeholder", `${rules.minFee} - ${rules.maxFee}`);
		$('#reason').attr("placeholder", `Mô tả chi tiết lí do giam giữ từ ${rules.minReasonLength} từ hoặc nhiều hơn`)
	}
	...
	function validateImpoundForm () {
		...
		if(String(weeks).length < 1 || parseInt(weeks) < 0 || parseInt(weeks) > rules.maxWeeks || String(days).length < 1 || parseInt(days) < reason.minDays || parseInt(days) > rules.maxDays || String(hours).length < 1 || parseInt(hours) < 0 || parseInt(hours) > rules.maxHours) {
			errors.append(`<small>&#9679; Số tuần phải là 0 hoặc ít hơn ${rules.maxWeeks}, ngày là 0 hoặc ít hơn ${rules.maxDays} và số giờ là 0 hoặc ít hơn ${rules.maxHours}.</small>`);
			success = false;
		}

		if(fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
			errors.append(`<small>&#9679; Phí không được thấp hơn ${rules.minFee} hoặc nhiều hơn ${rules.maxFee}</small>`);
			success = false;
		}

		if(reason.length < rules.minReasonLength || reason.length > 10000) {
			errors.append(`<small>&#9679; Lý do giam giữ không được dài hơn ${rules.minReasonLength} kí tự.</small>`);
			success = false;
		}
